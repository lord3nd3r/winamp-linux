...
float EqualizerWindow::getBandGainDb(int band) const {
    if (band < 0 || band >= 10) return 0.0f;
    if (!eqEnabled) return 0.0f;
    return (eqValues[band] - 32) * 12.0f / 32.0f;
}

float EqualizerWindow::getPreampGainDb() const {
    if (!eqEnabled) return 0.0f;
    return (preampValue - 32) * 12.0f / 32.0f;
}

int EqualizerWindow::getBandValue(int band) const {
    if (band < 0 || band >= 10) return 31;
    return eqValues[band];
}

int EqualizerWindow::getPreampValue() const { return preampValue; }

bool EqualizerWindow::isEnabled() const { return eqEnabled; }

bool EqualizerWindow::isAutoEnabled() const { return autoEnabled; }

bool EqualizerWindow::isShadeMode() const { return shadeMode; }


void EqualizerWindow::autoLoadPreset(const QString &filePath) {
...
void EqualizerWindow::toggleShadeMode() {
    shadeMode = !shadeMode;
    if (shadeMode) {
        setFixedSize(275, 14);
    } else {
