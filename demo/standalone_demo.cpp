...
protected:
    void paintEvent(QPaintEvent *) override {
        ...
        if (player->playbackState() == QMediaPlayer::PlayingState) {
            ...
            }TitleButton(QPainter &p, int x, int y, const QString &text, const QColor &color) {
    ...
    void drawVolumeSlider(QPainter &p, int x, int y, int vol) {
        ...
        p.fillRect(x, y + 12, volWidth, 4, volGrad, 110 + i * 3, 57 - y);
                }
            }
        }
    ...
    void mouseReleaseEvent(QMouseEvent *event) override {
        isDragging = false(QColor(230, 230, 230));
        p.setFont(QFont("Arial", 8, QFont::Bold));
        QRect textRect(x, y, 30, 18);
        p.drawText(textRect, Qt::AlignCenter, text);
    }
    ...
    void mousePressEvent(QMouseEvent *event) override {
        ...
        // Check button clicks (simplified hit detection)
        if (y >= 75 && y <= 93) {
            if (x >= 53 && x < 83) {  // Play button
                if (!currentFile.isEmpty()) {
                    player->play();
                } else {
                    openFile();
            ...
            else if (x >= 88 && x < 118) {  // Stop button
                player->stop();
    QPoint dragPosition;
    bool isDragging;
    int volume;
            }
            else if (x >= 123 && x < 153) {  // Pause button
                player->pause();
            }
            else if (x >= 205) {  // Open button
                openFile();
            }
        }
