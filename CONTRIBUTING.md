...
`project(Winamp VERSION 1.2.0 ...)` in `CMakeLists.txt` plus `WINAMP_VERSION_FULL` (`1.2.0`) and `kWinampVersion` in `src/constants.h` must stay aligned with **GitHub release tags**. Bump all three when cutting a release.
